...
//We used a hash table for store users by username and seperate chaining used to handle collisions//

public class UserHashTable {
   private static final int DEFAULT_CAPACITY = 16;
   private static final UserNode[] buckets;
   private final int capacity;
   private int size;

   //We used a node inside in each bucket
...
public boolean addUser(User user){
    ...
    if (getUser(user.getUsername()) != null){
        System.out.println ("Error : Username is already exists.");
        return false;
    }
